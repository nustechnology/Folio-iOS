...
final class NoteListViewModel: ViewModelProtocol {
  ...
  @Published private(set) var editTitle = ""
  @Published private(set) var editContent = ""
  ...
  func handle(_ action: Action) {
    ...
    case .dismissSheet, .dismissToast, .editTitleChanged, .editContentChanged:
...
extension NoteListViewModel {
  ...
  private func saveEdit(_ note: Note) {
    guard !editTitle.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty,
      !isSaving
    else {
      return
    }
  ...
  private func update(_ note: Note) async {
  ...
  private func delete(_ note: NoteSummary) async {
    ...
    do {
      ...
      invalidateInFlightListLoads()
      state.pendingDelete = nil
      if case .edit(let editingNote) = state.sheet, editingNote.id == note.id {
        setSheet(nil)
      }
